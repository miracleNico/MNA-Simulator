...
try:
    from krylov_solver import (
        krylov_solve_linear,
        adaptive_krylov_order,
        krylov_exp_mv,
    )
except Exception:
    krylov_solve_linear = None
    adaptive_krylov_order = None
    krylov_exp_mv = None

V_T = "0.025"
I_s = "1e-15"

# Krylov/iterative solve settings for large systems
SPARSE_THRESHOLD = 150
SOLVER_MODE = "auto"  # "auto" | "direct" | "krylov"
KRYLOV_DIM = 30       # reserved for future Arnoldi-style timestep approximation
KRYLOV_TOL = 1e-9
KRYLOV_MAXITER = 2000
KRYLOV_RESTART = 80
KRYLOV_ENABLE = True
KRYLOV_VERBOSE = False
TRANSIENT_METHOD = "backward_euler"  # "backward_euler" | "krylov_exponential"


def _should_use_krylov(A):
    if not KRYLOV_ENABLE or spla is None:
        return False
    if SOLVER_MODE == "direct":
        return False
    if SOLVER_MODE == "krylov":
        return True
    return A.shape[0] >= SPARSE_THRESHOLD


def solve_linear_system(A, b, context="linear"):
    """
    Solve A x = b with direct solve by default and GMRES/ILU for large matrices.
    Returns x with the same dimensionality as b.
    """
    b_arr = np.array(b, dtype=np.complex128 if np.iscomplexobj(A) or np.iscomplexobj(b) else np.float64)
    b_was_2d_col = (b_arr.ndim == 2 and b_arr.shape[1] == 1)
    b_1d = b_arr.reshape(-1)

    if _should_use_krylov(A):
        if krylov_solve_linear is not None:
            try:
                x_krylov, info = krylov_solve_linear(
                    A,
                    b_arr,
                    tol=KRYLOV_TOL,
                ...
                if KRYLOV_VERBOSE:
                    print(f"[Krylov:{context}] method={info.get('method', 'unknown')}")
                return x_krylov
            except Exception as e:
                if KRYLOV_VERBOSE:
                    print(f"[Krylov:{context}] module solve failed ({e}), fallback local")
        ...
        try:
            ...
            try:
                ...
                M = spla.LinearOperator(A_csc.shape, matvec=ilu.solve)
            except Exception:
                M = None

            x_1d, info = spla.gmres(
                A_csc,
                b_1d,
                rtol=KRYLOV_TOL,
                atol=0.0,
                restart=min(KRYLOV_RESTART, max(5, A.shape[0])),
                maxiter=KRYLOV_MAXITER,
                M=M,
            )
            if info == 0:
                if KRYLOV_VERBOSE:
                    print(f"[Krylov:{context}] GMRES converged for {A.shape[0]}x{A.shape[1]}")
                return x_1d.reshape(-1, 1) if b_was_2d_col else x_1d
            if KRYLOV_VERBOSE:
                print(f"[Krylov:{context}] GMRES info={info}, fallback to direct")
        except Exception as e:
            if KRYLOV_VERBOSE:
                print(f"[Krylov:{context}] iterative path failed ({e}), fallback to direct")

    # Direct fallback (default for small matrices)
    x_direct = np.linalg.solve(A, b_arr)
    return x_direct


def configure_solver(mode="auto", sparse_threshold=150, krylov_dim=30,
                     krylov_tol=1e-9, transient_method="backward_euler",
                     verbose=False):
    """
    Configure solver behavior without changing function signatures across code.
    """
    global SOLVER_MODE, SPARSE_THRESHOLD, KRYLOV_DIM, KRYLOV_TOL
    global TRANSIENT_METHOD, KRYLOV_VERBOSE

    mode = (mode or "auto").lower()
    if mode not in ("auto", "direct", "krylov"):
        mode = "auto"

    tr_method = (transient_method or "backward_euler").lower()
    if tr_method not in ("backward_euler", "krylov_exponential"):
        tr_method = "backward_euler"

    SOLVER_MODE = mode
    SPARSE_THRESHOLD = max(1, int(sparse_threshold))
    KRYLOV_DIM = max(2, int(krylov_dim))
    KRYLOV_TOL = float(krylov_tol)
    TRANSIENT_METHOD = tr_method
    KRYLOV_VERBOSE = bool(verbose)

    print(
        f"[Solver Config] mode={SOLVER_MODE}, threshold={SPARSE_THRESHOLD}, "
        f"krylov_dim={KRYLOV_DIM}, tol={KRYLOV_TOL}, transient={TRANSIENT_METHOD}"
    )


def _apply_cli_solver_options():
    """
    Optional startup flags:
      --solver {auto|direct|krylov}
      --sparse-threshold N
      --krylov-dim K
      --krylov-tol TOL
      --transient-method {backward_euler|krylov_exponential}
      --krylov-verbose
      --validate-krylov
    """
    args = sys.argv[1:]
    ...
    verbose = KRYLOV_VERBOSE

    i = 0
    while i < len(args):
        token = args[i]
        if token == "--solver" and i + 1 < len(args):
            mode = args[i + 1]
            i += 2
        elif token == "--sparse-threshold" and i + 1 < len(args):
            sparse_threshold = int(args[i + 1])
            i += 2
        elif token == "--krylov-dim" and i + 1 < len(args):
            krylov_dim = int(args[i + 1])
            i += 2
        elif token == "--krylov-tol" and i + 1 < len(args):
            krylov_tol = float(args[i + 1])
            i += 2
        elif token == "--transient-method" and i + 1 < len(args):
            transient_method = args[i + 1]
            i += 2
        elif token == "--krylov-verbose":
            verbose = True
            i += 1
        elif token == "--validate-krylov":
            validate_krylov()
            sys.exit(0)
        else:
            i += 1
...
def validate_krylov():
    """
    Quick consistency check: compare direct vs Krylov on random systems.
    """
    global SOLVER_MODE
    print("--- Running Krylov validation ---")
    rng = np.random.default_rng(7)
    sizes = [40, 120, 240]
    old_mode = SOLVER_MODE
    old_threshold = SPARSE_THRESHOLD

    for n in sizes:
        A = rng.standard_normal((n, n))
        A = A.T @ A + 0.1 * np.eye(n)
        b = rng.standard_normal((n, 1))

        configure_solver(mode="direct", sparse_threshold=old_threshold, krylov_dim=KRYLOV_DIM, krylov_tol=KRYLOV_TOL, transient_method=TRANSIENT_METHOD, verbose=False)
        x_direct = solve_linear_system(A, b, context=f"validate_direct_{n}")

        configure_solver(mode="krylov", sparse_threshold=old_threshold, krylov_dim=KRYLOV_DIM, krylov_tol=KRYLOV_TOL, transient_method=TRANSIENT_METHOD, verbose=False)
        x_krylov = solve_linear_system(A, b, context=f"validate_krylov_{n}")

        rel = np.linalg.norm(x_direct - x_krylov) / (np.linalg.norm(x_direct) + 1e-30)
        print(f"[validate] n={n}: relative_error={rel:.3e}")

    configure_solver(
        mode=old_mode,
        sparse_threshold=old_threshold,
        krylov_dim=KRYLOV_DIM,
        krylov_tol=KRYLOV_TOL,
        transient_method=TRANSIENT_METHOD,
        verbose=KRYLOV_VERBOSE,
    )
    print("--- Krylov validation done ---")
...
##LU or NR solver
    ...
    for i in range(max_iter):
        ...
        try:
            ...
            delta_x = solve_linear_system(J_k, -F_k, context="dc_nr")
...
def solve_harmonic_balance(G, C, f_str_vector, b_dc, b_ac_str, omega_0, H, max_iter=100, tol=1e-6):
    ...
    if is_nonlinear(f_str_vector):
        ...
        try:
            x_bar = solve_linear_system(Y_bar, b_bar, context="hb_linear")
            return x_bar
    ...
    for iter_k in range(max_iter):
        ...
        # For every pair of nodes (i, j), there is a time-varying conductance g_ij(t).
        ...
        try:
            delta_x = solve_linear_system(J_total, -residual, context="hb_nr")
        except np.linalg.LinAlgError:
...
def solve_transient(G, C, f_str_vector, b_dc, b_ac_str,
    ...
    use_krylov_exp = (
        TRANSIENT_METHOD == "krylov_exponential"
        and (not is_nl)
        and (krylov_exp_mv is not None)
    )
    A_sys = None
    if use_krylov_exp:
        try:
            # dx/dt = A_sys x + C^{-1} b(t), with A_sys = -C^{-1}G
            A_sys = -np.linalg.solve(C, G)
            if KRYLOV_VERBOSE:
                print("[Transient] Using Krylov exponential stepper for linear system.")
        except Exception:
            use_krylov_exp = False
            print("[Transient] Krylov exponential unavailable for this system. Falling back to backward Euler NR.")
    ...
    for n in range(num_steps - 1):
        ...
        if use_krylov_exp:
            try:
                c_now = np.linalg.solve(C, b_now).reshape(-1)
                M_aug = np.zeros((size + 1, size + 1), dtype=A_sys.dtype)
                M_aug[:size, :size] = A_sys
                M_aug[:size, size] = c_now
                u0 = np.zeros((size + 1,), dtype=A_sys.dtype)
                u0[:size] = x_n.flatten()
                u0[size] = 1.0
                x_next = krylov_exp_mv(M_aug, u0, t_step, k=KRYLOV_DIM)
                results[:, n + 1] = np.real(x_next[:size])
                continue
            except Exception:
                # Fallback to existing BE/NR path on any failure
                use_krylov_exp = False
                print(f"[Transient] Krylov exponential failed at step {n}, switching to backward Euler NR.")
        ...
        for i in range(max_iter):
            ...
            try:
                delta_x = solve_linear_system(J_k, -F_k, context="transient_nr")
            except np.linalg.LinAlgError:
                error_handler(f"JACOBIAN_SINGULAR: Jacobian singular at t={t_new:.2e}, iter={i}")
                converged = False
                break
...
def print_help_message():
    ...
    print("\n--- Simulation Command ---")
    print(".dc : get dc operating points")
    print(".tran <time/s> (<time_step/s>) : Transient Waveform")
    ...
    print(".end : End Netlist Mark")
    print("\n--- Optional CLI Solver Flags (Detailed) ---")
    print("Use these flags when launching the script, for example:")
    print("  python mna_simulation.py --solver krylov --krylov-verbose")
    print("")
    print("1) --solver {auto|direct|krylov}")
    print("   Selects linear solve path used by Newton/HB/transient matrix solves.")
    print("   - auto   : use direct for small systems, Krylov for large systems (default)")
    print("   - direct : always force dense direct solve")
    print("   - krylov : always force Krylov/iterative solve")
    print("")
    print("2) --sparse-threshold <N>")
    print("   Matrix dimension threshold used when --solver auto is active.")
    print("   If system size >= N, Krylov path is selected. Default: 150.")
    print("   Example: --sparse-threshold 300")
    print("")
    print("3) --krylov-dim <K>")
    print("   Krylov subspace dimension for exponential transient stepping.")
    print("   Higher K can improve accuracy but increases cost. Default: 30.")
    print("   Example: --krylov-dim 40")
    print("")
    print("4) --krylov-tol <TOL>")
    print("   Relative tolerance used by GMRES iterative solver.")
    print("   Smaller value = tighter convergence, potentially slower. Default: 1e-9.")
    print("   Example: --krylov-tol 1e-10")
    print("")
    print("5) --transient-method {backward_euler|krylov_exponential}")
    print("   Selects .tran integrator.")
    print("   - backward_euler      : classic implicit step (default)")
    print("   - krylov_exponential  : matrix-exponential-based step for linear systems")
    print("   Note: nonlinear transient still uses Newton iterations.")
    print("")
    print("6) --krylov-verbose")
    print("   Enables solver diagnostics (selected path, GMRES status, fallbacks).")
    print("")
    print("7) --validate-krylov")
    print("   Runs a built-in consistency test (direct vs Krylov) and exits.")
    print("")
    print("--- CLI Examples ---")
    print("A) Force direct solver:")
    print("   python mna_simulation.py --solver direct")
    print("B) Auto mode with larger threshold:")
    print("   python mna_simulation.py --solver auto --sparse-threshold 300")
    print("C) Krylov transient test:")
    print("   python mna_simulation.py --solver krylov --transient-method krylov_exponential --krylov-verbose")

    print("\n--- Example Netlist (for --entire paste) ---")
    print("""
          V1 n1 0 FUNC 20m 0.5*np.exp(-t)
          R1 n1 n2 1k
          D1 n2 n3 1e-14
          C1 n2 n4 1u
          C2 n3 0 1u
...
if __name__ == "__main__":
    _apply_cli_solver_options()
    print("MNA simulator v0.53")
                        ...
                        case "HB_ANALYSIS":
                            ...
                            #Spectrum
                                ...
                                for i in range(num_nodes):
                                    ...
                                    if np.any(mask):
                                        if max(freqs[mask]) > max_freq:
                                            max_freq = max(freqs[mask])
                                        plt.bar(freqs[mask], mags[mask], width=f0/50,label=f"V({node_name[0]})")

                                plt.xlabel('Frequency (Hz)')
                                plt.title(f'Harmonic Spectrum (Base f0={f0:.2e}Hz)')
                                plt.xlim(0, 1.2*max_freq)
