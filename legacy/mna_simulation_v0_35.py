# -*- coding: utf-8 -*-
"""MNA_simulation_v0.34.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1t4yPLsp5llhPxOPCMxxKVeV619ykGICd
"""
...
def error_handler(message):
    ...
    elif code in ("NR_CONVERGENCE", "JACOBIAN_SINGULAR", "SOLVER_RETRY"):
        #numerical failures
...
def lu_decomposition(A):
    ...
    for k in range(n):
        ...
        if ill_conditioned(L):
            return None, None

        if np.linalg.det(A) == 0:
            error_handler("LU_CONVERGENCE: Matrix is singular.")
...
def solve_dc_nr(G, f_str_vector, b, max_iter=1000, v_tol=1e-9, f_tol=1e-9, init_cond = "DEFAULT"):
...
##LU or NR solver
    ...
    for i in range(max_iter):
        ...
        try:
            ...
            print(f"JACOBIAN_SINGULAR: Jacobian is singular at iteration {i}. Solver failed.")
            return None
        ...
        if norm_F_now > norm_F_prev: ##delta_x attenuation
            for j in range(25):
                ...
                if j == 25:
                    print(f"Warning: Solver did not improve after 25 attenuations.")
...
def MNA_generation(netlist_obj):
    ...
    for comp in netlist_obj.get_components():
        ...
        if comp_type == 'R':
            ...
            if comp.subtype == 'DC':
                ...
                pass
                ## To be implemented, arbitrary curren source
...
def solve_transient(G, C, f_str_vector, b_dc, b_ac_str,
                      t_stop, t_step,
                      max_iter=10000, v_tol=1e-9, f_tol=1e-9):
    ...
    if is_nl:
        try:
            ...
            error_handler(f"Error: Failed to compile nonlinear functions: {e}")
            return None, None
    else:
    ...
    for n in range(num_steps - 1):
        ...
        for i in range(max_iter):
            ...
            if L is None:
                error_handler(f"Error: Jacobian singular at t={t_new:.2e}, iter={i}")
                converged = False
                break
        ...
        if not converged:
            error_handler(f"Error: NR failed to converge at t={t_new:.2e} s")
...
def print_help_message():
    ...
    print("="*50)
    print("\nThis simulator parses a circuit netlist to generate MNA matrices.")
    ...
    print("  Ix <Node1> <Node2> <DC/SIN/COS/B> <Value>|(<Amp>,<Freq>)|<'expression'>")
    print("  Vx <Node1> <Node2> <DC/SIN/COS/B> <Value>|(<Amp>,<Freq>)|<'expression'>")
...
def get_and_validate_netlist():
    ...
    while True:
        ...
        mode_line = input("Select input mode (--entire, --help, or press Enter for line-by-line): \n")
        mode = mode_line.strip().lower()
        simulation_mode = "SHOW_MATRIX" # Changed from CHECK_MNA to SHOW_MATRIX for consistency
        ...
        try:
            ...
            if mode == '--entire':
                print("Paste Netlist(end with '.end' on a new line)")
                while True:
                    line = input()
                    if line.strip().lower() == ".end":
                        break
                    if line.strip() == "": #ignore empty lines in paste
                        continue
                    ...
                    if line.strip().lower().startswith(".tran"):
                        ...
                        if len(tran_param) == 3 :
                           ...
                           error_handler("Transient analysis parameter error. Use .tran <time> (<step_time>).")

                    parts = line.split()
                    comp = Component(parts)
                    cur_netlist.add_component(comp) # This line now handles errors

            else:
                #line by line
                print("Enter Circuit Netlist Lines (type '.end' to finish)")
...
if __name__ == "__main__":
    print("MNA simulator v0.35")
    print("Enter --help for HELP")
    print("Enter --entire for PASTING MNA NETLIST")
    print("Format: <Name> <Node1> <Node2> ... [Value]")
    print("For sources, high voltage side is in <Node1>")
    print("Nodes must be '0' or 'nx' (e.g., n1, n2).")
    print("-" *45)
    [final_netlist, simulation_mode, simulation_param] = get_and_validate_netlist()
    [G,C,f_str,b_dc,b_ac_str,num_nodes] = MNA_generation(final_netlist)
    ...
    match simulation_mode:
        case "SHOW_MATRIX": #Default changed to show matrix
            for mat in [G,C,f_str,b_dc,b_ac_str]:
                print(np.matrix(mat))
        case "DC_ANALYSIS":
            print(f"DC analysis x result: \n{solve_dc_nr(G,f_str,b_dc)}'") # Use b_dc
        case "TRAN_ANALYSIS":
            [t_points, results] = solve_transient(G, C, f_str, b_dc, b_ac_str,
                simulation_param[0], simulation_param[1])
            #print(t_points)
            #print(results)
            plt.figure
            for i in range(num_nodes):
                plt.plot(t_points, results[i, :], label=f"{final_netlist.get_components()[i].name}")
            plt.xlabel('Time (s)')
